 ...
 * @return string
 */
function foxer_wrap_embed_html( $html ) {
 ...
 * @return string
 */
function foxer_maybe_wrap_embed( $html, $url ) {
	...
	$patterns = apply_filters( 'foxer_maybe_wrap_embed_patterns', $patterns );

	foreach ( $patterns as $pattern ) {
		$do_wrap = preg_match( $pattern, $url );

		if ( $do_wrap ) {
			return foxer_wrap_embed_html( $html );
		}
	}

	return $html;
}
add_filter( 'embed_oembed_html', 'foxer_maybe_wrap_embed', 10, 2 );
